...
namespace SpecFlowProject1.Actions
{
    public class UICommonActions
    {
        ...
        public string GetTitle( int timeout = 0)
        {
            try
            {
                _driver.Navigate().GoToUrl(UICommonSelenium.url);
        ...
        public void WaitAndClick(string locator, int timeout = 0)
        {
            if (timeout != 0)
            {
                WaitforVisibilityOfElement(locator, timeout);
        ...
        {
            if (timeout != 0)
                WaitforVisibilityOfElement(primaryMenu, 3);
            //Doing MouseHover
